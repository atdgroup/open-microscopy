...
static int zdrive_set_datum (CoarseZDrive* zd)
{
	...
	if(ftdi_controller_i2c_write_bytes(zd->controller, 0x42, 6, vals) != FT_OK) {
...
{ 
	...
	zd->_full_range_um = full_range_um;
...
static int zdrive_read_position(CoarseZDrive* zd, int *position, unsigned int *counts, unsigned int *stop_ind)
{ 
	byte vals[10] = "";  
	unsigned int cmd,msb,lsb_2,lsb_1,lsb;
	...
	if(ftdi_controller_i2c_read_bytes(zd->controller, 0x42, 5, vals) != FT_OK) {
		ftdi_controller_release_lock(zd->controller);
		*position = 0;
		*counts = 0;
		*stop_ind = 0;
    ...
    *stop_ind = vals[4] & 0xff;  
	...
	// The scale count of the device is arounf 4 billion so rob has add half scale.
...
{
	double range, position, start_time;
	unsigned int counts;
...
{
	int value, lsb, msb, inc_speed;
	int cmd = 10;
	char vals[10] = "";
	double backlash_nm;

	double backlash_nn = backlash_um * 1000.0;
	unsigned int backlash_counts = (unsigned int) (backlash_nn /  zd->_calibration);
...
{
	int value, lsb, msb, minimum_speed;
...
static  int CVICALLBACK OnOpticalLiftClosedClicked (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
static int CVICALLBACK OnOpticalLift_ClosePressed (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
static int CVICALLBACK OnOpticalLift_SetupPressed (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
static int atd_b_zdrive_on_timer_tick (CoarseZDrive* zd)
{
	...
	GCI_Signal_Emit(UIMODULE_SIGNAL_TABLE(zd), "CoarseZDrivePositionChanged", GCI_VOID_POINTER, zd); 
}
...
static int atd_coarse_zdrive_hardware_device_initialise (HardwareDevice* device)
{
	...
	ftdi_controller_open(zd->controller, "DPCXXI5Q");
		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
		{
            ...
            if (FileSelectPopup (UIMODULE_GET_DATA_DIR(zd), zd->_settings_file, "*.ini", "Save", VAL_SAVE_BUTTON, 0, 1, 1, 1, path) <= 0)
				return 0;
	
			hardware_save_state_to_file(HARDWARE_DEVICE_CAST(zd), path, "w");
...
int coarse_z_drive_initialise(CoarseZDrive* zd)
{
	...
	InstallCtrlCallback (zd->_panel_id, MAIN_PNL_SETUP, OnOpticalLift_SetupPressed, zd);
	InstallCtrlCallback (zd->_panel_id, MAIN_PNL_CLOSE, OnOpticalLift_ClosePressed, zd);
	...
	InstallCtrlCallback (zd->_setup_panel_id, SETUP_PNL_CLOSE, OnOpticalLiftClosedClicked, zd);
...
int atd_coarse_zdrive_hardware_load_state_from_file (HardwareDevice* device, const char* filepath)
{
	CoarseZDrive* zd = (CoarseZDrive*)device;
	dictionary* d = NULL;
	int file_size, num_devices, int_val;    
	char buffer[500] = "";
	double double_val;
	...
	if(d != NULL) {
		...
		int_val = dictionary_getint(d,  dictionary_get_section_key(buffer, UIMODULE_GET_NAME(zd), "Nudge"), -1.0); 
		...
		int_val = dictionary_getint(d,  dictionary_get_section_key(buffer, UIMODULE_GET_NAME(zd), "ApproachDistance"), -1.0); 
		...
		int_val = dictionary_getint(d,  dictionary_get_section_key(buffer, UIMODULE_GET_NAME(zd), "FullRangeTravel"), -1.0); 
		...
		int_val = dictionary_getint(d,  dictionary_get_section_key(buffer, UIMODULE_GET_NAME(zd), "Backlash"), -1.0); 
