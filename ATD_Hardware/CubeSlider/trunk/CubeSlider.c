    ...
    switch (event)
    {
        case EVENT_TIMER_TICK:
        {
			...
			if(pos != cube_manager->_current_pos) {
				...
				GCI_Signal_Emit_From_MainThread(UIMODULE_SIGNAL_TABLE(cube_manager), "FluoCubeChanged", GCI_VOID_POINTER, cube_manager, GCI_INT, pos); 		
...
{
	int i, size = 0;
	FluoCube * device_node = NULL;
	FluoCube * cubes = NULL;
	CMDeviceNode *tmp = NULL;
	ListType list;

	size = device_conf_get_num_active_devices(DEVICE_CONF_CAST(cube_manager->dc));

	if(size < 1)
		return DEVICE_CONF_ERROR; 	

	list = device_conf_get_devices_in_use_list(DEVICE_CONF_CAST(cube_manager->dc));

	cubes = malloc(sizeof(FluoCube) * size);

	for(i=0; i < size; i++) {
	
		tmp = ListGetPtrToItem(list, i);  
		device_node = (FluoCube*) tmp->device;

		strcpy(cubes[i].name, tmp->name);
		cubes[i].position = tmp->position;
		cubes[i].exc_min_nm = device_node->exc_min_nm;
		cubes[i].exc_max_nm = device_node->exc_max_nm;
		cubes[i].emm_min_nm = device_node->emm_min_nm;
		cubes[i].emm_max_nm = device_node->emm_max_nm;
		cubes[i].dichroic_nm = device_node->dichroic_nm;
	}
...
int cube_manager_get_current_value_text(HardwareDevice* device, char* info)
{
	...
	if (item.name!=NULL){
		strncpy(info, item.name, CUBE_MANAGER_NAME_SIZE);
		return CUBE_MANAGER_SUCCESS;
	}
...
static int cube_manager_hardware_device_initialise (HardwareDevice* device)
{
	FluoCubeManager* cube_manager = (FluoCubeManager*) device;

	return cube_manager_hardware_initialise(cube_manager);
}
...
FluoCubeManager* cube_manager_new(const char *name, const char *description, const char *data_dir, const char *data_file, size_t size)
{
	...
	GciCmtNewLock("CubeManager", 0, &(cube_manager->_lock));
	...
	HARDWARE_VTABLE_PTR(HARDWARE_DEVICE_CAST(cube_manager), hardware_initialise) = cube_manager_hardware_device_initialise; 
...
int cube_manager_get_current_cube_position(FluoCubeManager* cube_manager, int *position)
{
	...
	do {
		...
		if( (cube_manager->vtable.get_current_cube_position)(cube_manager, position) == CUBE_MANAGER_ERROR ) {
			...
			if(hardware_device_get_number_of_consecutive_errors(HARDWARE_DEVICE_CAST(cube_manager)) >= max_errors) {
				return CUBE_MANAGER_ERROR;
			}

			status = send_fluocube_error_text(cube_manager, "cube_manager_get_current_cube_position failed");

			hardware_device_increment_consecutive_errors(HARDWARE_DEVICE_CAST(cube_manager));
	...
	hardware_device_reset_consecutive_errors(HARDWARE_DEVICE_CAST(cube_manager));

  	return CUBE_MANAGER_SUCCESS;
}


static int cube_manager_keep_checking_for_cube_position_reached_with_timeout(FluoCubeManager* cube_manager, int position, double timeout)
{
	double start_time = Timer();
	int pos=-1;
...
static int CVICALLBACK check_cube_moved_correctly(void *callback)
{
	...
	logger_log(UIMODULE_LOGGER(cube_manager), LOGGER_ERROR, "Cube slider failed to move within timeout");
...
static int cube_manager_move_virtual_method(FluoCubeManager* cube_manager, int position)
{
	return cube_manager->vtable.move_to_cube_position(cube_manager, position);
}

int cube_manager_move_to_position(FluoCubeManager* cube_manager, int position)
{
	int check_move_return_value, other_pos, status = UIMODULE_ERROR_NONE;
	double start_time = Timer();

	GciCmtGetLock(cube_manager->_lock);

	#ifdef VERBOSE_DEBUG
		printf("cube_manager_move_to_position 1\n");
	#endif
	...
	#ifdef VERBOSE_DEBUG
		printf("cube_manager_move_to_position 2\n");
	#endif
	...
	#ifdef VERBOSE_DEBUG
		printf("cube_manager_move_to_position 3\n");
	#endif
	...
	#ifdef VERBOSE_DEBUG
		printf("cube_manager_move_to_position 4\n");
	#endif

	if (!hardware_device_is_manual((HardwareDevice *)cube_manager)){
		...
		if (cube_manager==NULL || ABORT_ALL)  // cabe manager has been destroyed whilst waiting, maybe user quit
			return CUBE_MANAGER_ERROR;

		CmtGetThreadPoolFunctionAttribute (gci_thread_pool(),  cube_manager->_move_pos_thread_id, ATTR_TP_FUNCTION_RETURN_VALUE, &check_move_return_value);

	#ifdef VERBOSE_DEBUG
		printf("cube_manager_move_to_position 5\n");
	#endif

		CmtReleaseThreadPoolFunctionID(gci_thread_pool(),  cube_manager->_move_pos_thread_id);
	}
	else {  // for manual device
		check_move_return_value = CUBE_MANAGER_SUCCESS;
	}

	GCI_Signal_Emit_From_MainThread(UIMODULE_SIGNAL_TABLE(cube_manager), "FluoCubeChanged", GCI_VOID_POINTER, cube_manager, GCI_INT, position); 		

	cube_manager->_current_pos = position;
				
	#ifdef VERBOSE_DEBUG
	printf("cube_manager_move_to_position 6\n");
	#endif
	...
	#ifdef VERBOSE_DEBUG
		printf("cube_manager_move_to_position 7\n");
	#endif

	if(check_move_return_value == CUBE_MANAGER_ERROR) {

		// Try to recover cube by moving to a difference position and then moving back to the requested position.
		other_pos = position - 1;

		if(other_pos < 1)
			other_pos = position + 1;

		cube_manager_move_virtual_method(cube_manager, other_pos);
		Delay(3.0);
		cube_manager_move_virtual_method(cube_manager, position);
		Delay(3.0);
	...
	#ifdef VERBOSE_DEBUG
		printf("cube_manager_move_to_position 8\n");
	#endif

	
	cube_manager->_prevent_timer_callback = 0;  

	GciCmtReleaseLock(cube_manager->_lock);

	#ifdef VERBOSE_DEBUG
		printf("cube_manager_move_to_position 9\n");
	#endif
