...
static int Manual_move_to_cube_position(FluoCubeManager* cube_manager, int position)
{
	...
	if (cube_manager_manual->stored_position == position)  // already at the position
		return CUBE_MANAGER_SUCCESS;

	cube_manager_manual->stored_position = position;

	hardware_device_get_current_value_text((HardwareDevice *)cube_manager, info);
	GCI_MessagePopup("Manual Cube Slider", "Please change the cube slider on the microscope to %s.", info);
