...
// for maxima finding
static int				x_points, y_points, regionGrowCount=0;
...
// non-max supression of the imageArray map
{
	...
	if (imageArray[pos]<imageArray[n_pos++]) return(0);
	if (imageArray[pos]<imageArray[n_pos++]) return(0);
	if (imageArray[pos]<imageArray[n_pos]) return(0);
	n_pos+=x_points;
	if (imageArray[pos]<imageArray[n_pos]) return(0);
...
static void FindOnlyMaxima(double threshold)
...
// the actual number of maxima found is returned.
{
	...
	if (pCentresIn!=NULL) pCentres = pCentresIn;

	if (threshold_in<0) threshold = 10.0;
