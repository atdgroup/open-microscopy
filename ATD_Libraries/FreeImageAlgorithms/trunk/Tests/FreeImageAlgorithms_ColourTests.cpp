...
TestFIA_ColourExtract(CuTest* tc)
{
	...
	FIBITMAP *R=NULL, *G=NULL, *B=NULL, *newdib=NULL, *newdib2=NULL;
	...
	error = FIA_ReplaceColourPlanesHSV(&newdib2, R, B, G);
	CuAssertTrue(tc, error == FIA_SUCCESS);
	FIA_SimpleSaveFIBToFile(newdib2, TEST_DATA_OUTPUT_DIR "/Colour/HSVColourReplaced.bmp"); 
	FreeImage_Unload(newdib2);
